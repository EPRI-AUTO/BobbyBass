...
class MotorSubscriberNode(Node):
    ...
    def robot_callback(self, msg):
        ...
        button_state_4 = self.joystick.get_button(2)
        ...
        if self.joystick.get_button(1) or self.status_message == "Current Status: Emergency Stop":
            self.robot.left_motor("STOP", 1)
            self.robot.right_motor("STOP", 1)
            self.robot.ind_light(2, "OFF")
            rclpy.shutdown()
        ...
        button_state_1 = self.joystick.get_button(3)
        ...
        button_state_2 = self.joystick.get_button(4)
        ...
        button_state_3 = self.joystick.get_button(5)
        ...
        if self.mode == "autonomous":
           ...
           # If the counter has been triggered too many times by the close signals, stop and back up (will change to actually redirect later)
            ...
            if self.nav_backup and ((current_time - self.commit_time) > 5):
                self.robot.left_motor("STOP", 1)
                self.robot.right_motor("STOP", 1)
                msg2 = String()
                msg2.data = "waypoint"
